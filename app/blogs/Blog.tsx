    ...
    const fetch = async () => {
      try {
        const { data } = await axios.get(
          "https://api.dentobees.com/landing"
        );
        setBlogData(data);
      } catch (error) {
        console.error(error);
  ...
  return (
    <div className="relative overflow-hidden z-0 bg-white">
      ...
      <div className="min-h-[900px] Common-padding pt-[110px] md:pt-[130px] xl:pt-[150px] pb-[90px] md:pb-[110px] flex flex-col items-center gap-6 md:gap-8">
        <h2 className="w-full Page-Heading text-center">Blogs</h2>
          ...
          {loading ?
            ...
            blogData.map((item, index) => (
              <div key={index} className="flex justify-center">
                <Link href={`/blogs/${item?.slug}`} className="w-full cursor-pointer Hover-effect">
                  <Card isPressable className="border-[1px] border-[#81D5B999] shadow-none flex flex-col gap-[18px] p-2.5 bg-[#FFFFFFB2] backdrop-blur-[19%]">
                    <CardBody className="p-0">
                      <Image src={item.image || "/default-image.jpg"} alt={item.articleTitle} width={390} height={250} className="w-full rounded-[16px] object-cover" />
                    </CardBody>
                    <CardFooter className="p-0 pb-3 flex flex-col gap-2.5 items-start font-bold">
                      <h5 className="h-[48px] md:h-[56px] Small-heading text-black text-left line-clamp-2">{item.articleTitle}</h5>
